...
| 🧠 **知识库（收集库）** | 所有文章与文字稿切成带出处的切片（哪一集 · 小标题 · 时间戳），中文分词 + 语义向量混合检索；新文章**生成完自动入库**，不用手点重建 |
| 💭 **记忆体系** | 让 AI 记住你：阅读页选中一句话点「记住这条」，或在知识库把结论存下来；之后每次提问自动带上，并**把用到的记忆显示出来**（看得见才纠正得了）。每条记忆都记着用过几次，从没用过的会被标出来，方便清理 |
...
**收集**：每生成一篇文章，它同时进知识库 —— 自动的，不用你点。侧边栏 **知识库** 里能做三件事：

- **一个搜索框搜两样**：资料（所有文章与文字稿的切片，带出处、可直接回听）和**你记过的**记忆
- **跨集提问**：就整个书库提问，回答只依据检索到的原文，每条依据标 `[n]`；这次用到了哪些记忆也会显示出来
- **实体**：人物 / 机构 / 媒体 / 话题榜，点进去看它在哪几集、哪一分钟出现过

**记住**：三种写法，都是**你说了才记**（不猜、不自动总结闲聊）：

1. 阅读页选中一段话 → **☆ 记住这条**（带上出自哪一集）
2. 助手回答或知识库回答下方 → **☆ 记住这个结论**
