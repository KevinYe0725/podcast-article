...
**解读只给答案，不交代「材料里有什么」**：不会出现「这一期的原文里没有覆盖这一点」「片段只讲到 A、B、C」这类清点材料的话——那是浪费阅读时间。需要标明一句不是本集原文时，只在句末标一个 `（原文未提及）`，不解释、不展开。（这条是用户反馈后改的：旧提示词恰恰**要求**它「第一句就写原文没有覆盖」，结果整个第一段都在讲材料。）
