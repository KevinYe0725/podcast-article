...
| 🔊 **朗读（TTS）** | 让 AI 把文章念出来：macOS 本地 `say`（免费·离线·无需密钥）或任何兼容 OpenAI `/v1/audio/speech` 的接口；正文清洗后分块生成，单块可重试，内容与音色一起哈希——没变就不重复花钱 |
| 🧠 **知识库（AI 的底座）** | 所有文章与文字稿切成带出处的切片（哪一集 · 小标题 · 时间戳），中文分词 + 语义向量混合检索；新文章**生成完自动入库**。界面上**没有**知识库页面 —— 它是给 AI 用的，你只需要在输入框里提问 |
| 💭 **记忆体系** | 让 AI 记住你：阅读页选中一句话点「记住这条」，或把某个结论存下来；之后每次提问自动带上，用到的记忆收在答案下的「AI 记得的你」里（看得见才纠正得了）。每条记忆都记着用过几次，从没用过的会被标出来，方便清理 |
...
**知识库是服务于 AI 的，不是让你去搜索的地方。** 所以界面上没有"知识库"这个页面，也没有让你手动
检索、翻实体榜的入口 —— 那是 AI 的底座，你不需要操作它。你只需要**一个输入框**：

| 你在输入框里做的事 | 会发生什么 |
|---|---|
| 粘一个链接 | 生成文章（照旧），并**自动**进知识库 —— 不用你点任何按钮 |
| 打一句话提问 | **问你的库**：AI 自己跨 12 集检索、带上你存过的记忆回答；回答里每条依据都带出处，时间戳可点回听 |

打字的时候输入框下面那行小字会告诉你这次是「🔗 将生成文章」还是「💬 问你的库」，按钮文案也跟着变，你按下去之前就知道结果。

提问的过程是**两段式**：先很快把「出处（哪几集、哪一段、哪一分钟）」列出来，再出答案 —— 不用对着空白等几十秒。

**记住**：三种写法，都是**你说了才记**（不猜、不自动总结闲聊）：

1. 阅读页选中一段话 → **☆ 记住这条**（带上出自哪一集）
2. 问完一个问题 → 答案下方 **☆ 记住这个结论**
3. 设置 → **记忆** 里手写；可以置顶（每次提问都带上它）、改、删

用到的记忆不会大剌剌摊在界面上（那同样是把内部机制推到你面前），而是在答案下面收成一个「AI 记得的你（N 条）」，
你想核对的时候展开看 —— **看得见才纠正得了**。

**记忆会腐化，所以这里做了几件事：**

- 每条记忆显示「用过 N 次 · 最近 M 天前」；`从未用过` 的会被标出来，可一键只看这些去清理
- 只有**真的对上号**的才算用过（置顶的、和问题相关的）；查询没命中时被顺手塞进上下文的不算
- 记忆**独立于索引**：重建索引、升级版本都不会动它（索引是派生数据、随时可重建；记忆不是）
- 删除是**真的删**：列表与检索里立刻消失

索引万一坏了要修：**设置 → 记忆 → 「知识库（AI 用的底座，日常不用管）」** 里能看到规模与语义检索状态，
也有重建索引按钮。放得深是故意的：它是维护入口，不是日常入口。

命令行一样能用：

```bash
uv run podcast-article search "推理芯片的成本"    # 跨集检索：哪一集 · 小标题 · 时间戳
uv run podcast-article ask "孙正义怎么押注的"      # 跨集问答（会调用模型，产生费用）
uv run podcast-article remember "我偏好看有数字的播客" --kind preference --pin
uv run podcast-article recall --never-used        # 清点从没用过的记忆
uv run podcast-article forget 3
uv run podcast-article status                     # 书库规模 / 语义检索是否可用 / 记忆条数
uv run podcast-article index --force              # 重建索引（会顺带补齐缺失的向量）
```

MCP 客户端（Claude Desktop、DSH 等）里对应 `search_library` / `ask_library` / `remember` / `recall` / `forget` 五个工具 —— 你在别的 AI 里也能让它「搜一下我播客库里关于 X 的内容」和「帮我记住这个」。

> 语义检索有个**相似度下限**（实测标定：相关 ≥0.60、无关 ≤0.56，取 0.58）。没有它的话，
> 余弦相似度对任何输入都有值，「完全无关的问题」也会拿到一堆貌似相关的原文去喂模型 —— 那比搜不到更坏。
> 只影响向量命中，词法命中不受影响。换嵌入模型后需要用 `PA_KB_SEM_MIN` 重新标定。

### 让文章被念出来（朗读）

阅读页工具条上有一个 **🔊 朗读**：点一下就把这篇念成音频，生成时显示进度（第几块 / 共几块），
好之后出现一个播放条（播放 / 暂停、点进度条跳转、0.75-2 倍速、重新生成、删除）。

两种后端，在 **设置 → 朗读** 里选：

| 后端 | 需要什么 | 说明 |
|---|---|---|
| **macOS 本地** | 什么都不用配 | 用系统自带的 `say`，免费、离线、不花钱；中文音色可选 Tingting / Sinji / Meijia 等（设置页会列出这台机器上有的音色，点一下即填） |
| **OpenAI 兼容接口** | `base_url` + `API 密钥` + 模型/音色 | 任何实现 `POST /v1/audio/speech` 的服务都能接：OpenAI、硅基流动、Groq、各家 OpenAI 中转站……密钥只写不读，存在 `.env` 的 `TTS_API_KEY` |

几个设计上的取舍：

- **分块生成**：TTS 接口普遍有单次字数上限（常见 4096），所以按段落/句子切成 ~700 字一块。
  好处不只是绕过上限——第 8 块失败时，前 7 块（已经花过钱的）不用重读。
- **按内容哈希缓存**：把「清洗后的正文 + 后端 + 模型 + 音色 + 语速」一起哈希，只有真的变了才重读。
  改一个字就整篇重念是最贵的用法，这里不干。
- **整篇 vs 分段**：有 ffmpeg 时把各块拼成一个文件（可任意拖进度条）；没有就按段连播，功能不受影响。
- **省钱提示**：先把音色试听好再生成整篇（设置页有「试听一句」）；换音色会让整篇重读一次，
  但**换播放倍速不会**（倍速是浏览器端的）。
- 只读镜像（公网部署）上朗读的生成接口是关掉的（要密钥、要花钱），但已有音频能正常播放。
