...
| 档位 | 结构 | 实测篇幅（109 分钟访谈） |
|---|---|---|
| 精华 | 3 节 + 带走清单 | 约 3600 字 / 7 分钟 |
| 标准 | 4-5 节 + 编辑点评 | 约 6000 字 / 12 分钟 |
| 深度 | 再加书影音表格与金句摘录 | 8000 字以上 |

> 说明：LLM 无法被指令精确控制字数（实测 5 轮，产出稳定为目标值的 1.5-2 倍），所以我们**不假装控制**：档位负责结构与详略，格式纪律交给确定性后处理（`postprocess.py`：拆超长段落、修孤立时间戳、删套话、规范中文标点），篇幅只在极端超长时（超过上限 1.6 倍）机械兜底。
...
| 生成默认值 | 默认篇幅档位、默认转写语言、转写后端、ASR 模型、直读上限、是否强制转写、生成后是否复检 |
| MCP 服务器 | 见下节 |
...
├── postprocess.py    # 确定性排版收尾（段落/标点/套话/时间戳）
