...
- **凡是内容不是来自本集原文的句子，都必须在句末标一次 `（原文未提及）`** —— 这是硬性要求，不是可选项。
  实测写成「需要时标一下」时，模型会整篇不打标记，于是读者分不清哪些是播客内容、哪些是它自己的知识。
- 来自网络资料时标 `（据网络资料）`，不要说来源性质、不要提示可能过时
- 这两个标记**字数极少、读者界面上另有一处收着**（正文里几乎不占位置），所以该标就标，不要因为怕啰嗦而省掉
- 标记只写四个字，**不要**在它前后加任何解释（不要写「这一点原文没有提到，我补充一下」）
